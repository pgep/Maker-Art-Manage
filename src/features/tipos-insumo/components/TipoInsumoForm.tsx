...
import { TipoInsumo, TipoInsumoFormData } from '../../../core/types.ts';
...
interface TipoInsumoFormProps {
  ...
  onSave: (data: TipoInsumoFormData) => Promise<void>;
  onCancel: () => void;
  isSaving: boolean;
}

export const TipoInsumoForm: React.FC<TipoInsumoFormProps> = ({
  tipoInsumo,
  onSave,
  onCancel,
  isSaving,
  ...
  useEffect(() => {
    if (tipoInsumo) {
      setNome(tipoInsumo.nome);
      setAtivo(tipoInsumo.ativo);
    } else {
      setNome('');
      setAtivo(true);
    }
  ...
  const validateField = (field: string, value: string): string | null => {
    if (field === 'nome') {
      ...
      if (!trimmed) {
        return 'O nome do tipo de insumo é obrigatório.';
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              {isEditing ? 'Editar Tipo de Insumo' : 'Novo Tipo de Insumo'}
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              {isEditing
                ? `Atualize as informações do tipo de insumo #${tipoInsumo?.id}`
                : 'Cadastre uma nova classificação genérica para matérias-primas e insumos'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            {isEditing ? 'Salvar Alterações' : 'Salvar Tipo de Insumo'}
          </Button>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            <Input
              ...
              placeholder="Ex.: Tecidos, Madeiras, Tintas, Fios, Embalagens..."
              error={touched.nome ? errors.nome : undefined}
              required
              maxLength={100}
              helpText="Nome único que categoriza os insumos e matérias-primas. Máximo 100 caracteres."
            />
          </div>
            ...
            <div className="p-4 bg-slate-50 rounded-lg border border-slate-200/80 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  {ativo
                    ? 'Este tipo de insumo está atualmente ATIVO.'
                    : 'Este tipo de insumo está atualmente INATIVO.'}
                </p>
              </div>
              <span
