...
interface ListProps {
  ...
  onToggleStatus: (tipoInsumo: TipoInsumo) => void;
  onDelete: (tipoInsumo: TipoInsumo) => void;
  onRefresh: () => void;
}
...
export const TipoInsumoList: React.FC<ListProps> = ({
  tiposInsumo,
  total,
  currentPage,
  pageSize,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Gerencie as categorias e classificações de insumos e matérias-primas do ateliê
          </p>
        </div>
      ...
      <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-3">
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3">
          ...
          <div className="flex-1 relative">
            <Input
              id="tipo-insumo-search"
              placeholder="Buscar tipo de insumo por nome..."
              value={searchTerm}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden">
        {error ? (
          <ErrorState
            title="Não foi possível carregar os tipos de insumo"
            message={error}
            onRetry={onRefresh}
          />
        ) : isLoading ? (
          ...
          <EmptyState
            title={isFilterActive ? 'Nenhum tipo de insumo encontrado' : 'Nenhum tipo de insumo cadastrado'}
            description={
              isFilterActive
                ? 'Tente ajustar os termos da pesquisa ou alterar o filtro de status.'
                : 'Cadastre os tipos de insumos para classificar os materiais e matérias-primas utilizados no ateliê.'
            }
            actionLabel={isFilterActive ? 'Limpar Filtros' : 'Novo Tipo de Insumo'}
            onAction={
              isFilterActive
                ? () => {
                    onSearchChange('');
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            ...
                            title={item.ativo ? 'Inativar tipo de insumo' : 'Ativar tipo de insumo'}
                          >
                            {item.ativo ? (
