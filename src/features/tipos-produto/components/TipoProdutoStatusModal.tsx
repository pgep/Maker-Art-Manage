...
interface TipoProdutoStatusModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export const TipoProdutoStatusModal: React.FC<TipoProdutoStatusModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  tipoProduto,
  ...
  const isCurrentlyActive = tipoProduto.ativo;
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isCurrentlyActive ? 'Inativar Tipo de Produto' : 'Ativar Tipo de Produto'}
      size="sm"
    >
      <div className="space-y-4">
        <div className="flex items-start gap-3.5">
          ...
          <div className="text-xs text-slate-600 space-y-1.5 leading-relaxed">
            <p>
              Deseja realmente {nextAction} o tipo de produto{' '}
            ...
            {isCurrentlyActive ? (
              ...
              <p className="text-slate-500 text-[11px] bg-slate-50 p-2.5 rounded-lg border border-slate-200/70">
                <span className="font-semibold text-slate-700">Nota:</span> Ao ativar, este tipo de produto voltará a ficar disponível na listagem padrão de ativos.
              </p>
            )}
          </div>
        </div>
